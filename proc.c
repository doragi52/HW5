...
int clone(void(*fcn)(void*,void*), void *arg1, void *arg2, void* stack)
{
  ...
  np->pgdir = mp->pgdir;
  np->sz = mp->sz; //size of process memory
  np->parent = mp;
  *np->tf = *mp->tf; //trapframe
...
join(void** stack)
{
  ...
  acquire(&ptable.lock);
  for(;;){
      // scan table to find children.
      child = 0;
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){

      if(p->pgdir != p->parent->pgdir||p->parent != mp)
        continue; // is not a thread
      ...
      if(p->state == ZOMBIE){
        ...
        // Reset thread from the process table
        p->pid = 0;
        p->parent = 0;
        p->name[0] = 0;
        p->killed = 0;
